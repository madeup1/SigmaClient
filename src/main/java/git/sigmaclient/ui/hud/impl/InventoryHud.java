...
import git.sigmaclient.modules.render.InventoryDisplay;
...
public class InventoryHud extends DraggableComponent
{
    ...
    public InventoryHud() {
        ...
        this.setPosition(SigmaClient.inventoryDisplay.x.getValue(), SigmaClient.inventoryDisplay.y.getValue());
    }

    public HudVec drawScreen() {
        GL11.glPushMatrix();
        super.drawScreen();
        final ScaledResolution scaledResolution = new ScaledResolution(SigmaClient.mc);
        int blur = 0;
        ...
        final String selected = SigmaClient.inventoryDisplay.blurStrength.getSelected();
        ...
        for (int i = 9; i < 36; ++i) {
            ...
            final ItemStack stack = SigmaClient.mc.thePlayer.inventory.getStackInSlot(i);
            if (stack != null) {
                SigmaClient.mc.getRenderItem().renderItemAndEffectIntoGUI(stack, (int)x + 2 + 20 * (i % 9), (int)y);
        ...
        SigmaClient.inventoryDisplay.x.set(this.x);
        SigmaClient.inventoryDisplay.y.set(this.y);
