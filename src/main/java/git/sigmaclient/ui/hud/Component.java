package git.sigmaclient.ui.hud;
...
public class Component
{
    ...
    protected static int getMouseX() {
        return Mouse.getX() * getScaledResolution().getScaledWidth() / SigmaClient.mc.displayWidth;
    }

    protected static int getMouseY() {
        final int height = getScaledResolution().getScaledHeight();
        return height - Mouse.getY() * height / SigmaClient.mc.displayHeight - 1;
    }

    protected static ScaledResolution getScaledResolution() {
        return new ScaledResolution(SigmaClient.mc);
    }
