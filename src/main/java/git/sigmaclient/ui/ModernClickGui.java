package git.sigmaclient.ui;

import git.sigmaclient.SigmaClient;
import git.sigmaclient.managers.WindowManager;
import git.sigmaclient.modules.Module;
import git.sigmaclient.modules.render.PopupAnimation;
...
import git.sigmaclient.utils.render.GLUtils;
...
public class ModernClickGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        ScaledResolution sr = new ScaledResolution(SigmaClient.mc);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        switch(SigmaClient.clickGui.blur.getSelected()) {
            case "Low":
        ...
        RenderUtils.drawBorderedRoundedRect((float) getX() - 5, (float) getY() - 5, getWidth() + 10, getHeight() + 10, 3.0f, 2 , SigmaClient.themeManager.getPrimaryColor().getRGB(), SigmaClient.themeManager.getSecondaryColor().getRGB());
        RenderUtils.drawBorderedRoundedRect((float)getX(), (float)getY(), 85.0f, getHeight(), 3.0f, 2.0f, SigmaClient.themeManager.getPrimaryColor().getRGB(), SigmaClient.themeManager.getSecondaryColor().getRGB());
        RenderUtils.drawBorderedRoundedRect((float)(getX() + 90.0), (float)getY(), getWidth() - 90.0f, 20.0f, 3.0f, 2.0f, SigmaClient.themeManager.getPrimaryColor().getRGB(), SigmaClient.themeManager.getSecondaryColor().getRGB());
        RenderUtils.drawBorderedRoundedRect((float)(getX() + 90.0), (float)(getY() + 25.0), getWidth() - 90.0f, getHeight() - 25.0f, 3.0f, 2.0f, SigmaClient.themeManager.getPrimaryColor().getRGB(), SigmaClient.themeManager.getSecondaryColor().getRGB());
        Fonts.getSecondary().drawCenteredString("SigmaClient (v"+ SigmaClient.VERSION+")", (float)(getX() + 42.5), (float)(getY() + 6.0), Color.WHITE.getRGB());
        ...
        for (Window window : this.windowManager.windows) {
            ...
            if(window.getName().equals("Settings")) {
                if (window == selectedWindow) {
                    RenderUtils.drawBorderedRoundedRect((float)(getX() + 5.0), (float)(getY() + (double)settingsOffset + 3.0), 75.0f, 12.0f, 4.0f, 4.0f, SigmaClient.themeManager.getSecondaryColor().getRGB(), SigmaClient.themeManager.getSecondaryColor().getRGB());
                }
                Fonts.getPrimary().drawStringWithShadow(window.getName(), getX() + 12.0, getY() + (double)settingsOffset + 5.0, Color.WHITE.getRGB());
            } else {
                if (window == selectedWindow) {
                    RenderUtils.drawBorderedRoundedRect((float)(getX() + 5.0), (float)(getY() + (double)categoryOffset + 3.0), 75.0f, 12.0f, 4.0f, 4.0f, SigmaClient.themeManager.getSecondaryColor().getRGB(), SigmaClient.themeManager.getSecondaryColor().getRGB());
                }
            ...
            RenderUtils.drawBorderedRoundedRect((float)ModernClickGui.getX() + 88.0f, (float)ModernClickGui.getY() + 25.0f, ModernClickGui.getWidth() - 88.0f, ModernClickGui.getHeight() - 25.0f, 6.0f, 2.0f, SigmaClient.themeManager.getPrimaryColor().getRGB(), SigmaClient.themeManager.getPrimaryColor().getRGB());
    ...
    public void onGuiClosed() {
        ...
        SigmaClient.clickGui.setToggled(false);
    }
