...
public class SkyblockUtils {
    ...
    public void onTick(final TickEvent.ClientTickEvent event) {
        if (SigmaClient.mc.theWorld != null && event.phase == TickEvent.Phase.START) {
    ...
    {
        return SigmaClient.mc.getCurrentServerData().serverIP.endsWith("hypixel.net");
    }

    public static boolean isOnSkyBlock() {
        try {
            final ScoreObjective titleObjective = SigmaClient.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1);
            if (SigmaClient.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(0) != null) {
                return ChatFormatting.stripFormatting(SigmaClient.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(0).getDisplayName()).contains("SKYBLOCK");
            }
            return ChatFormatting.stripFormatting(SigmaClient.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1).getDisplayName()).contains("SKYBLOCK");
        }
        catch (Exception e) {
            return false;
        }
    ...
    public static boolean isInOtherGame() {
        try {
            final Scoreboard sb = SigmaClient.mc.thePlayer.getWorldScoreboard();
            final List<Score> list = new ArrayList<Score>(sb.getSortedScores(sb.getObjectiveInDisplaySlot(1)));
            for (final Score score : list) {
                final ScorePlayerTeam team = sb.getPlayersTeam(score.getPlayerName());
    ...
    {
        if (SigmaClient.mc.thePlayer == null) return new ArrayList<>();
        return playerInfoOrdering.sortedCopy(SigmaClient.mc.thePlayer.sendQueue.getPlayerInfoMap());
    }
