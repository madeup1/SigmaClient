...
public class EntityUtils
{
    private static boolean isOnTeam(final EntityPlayer player) {
        for (final Score score : SigmaClient.mc.thePlayer.getWorldScoreboard().getScores()) {
    ...
    public static boolean isTeam(final EntityLivingBase e2) {
        ...
        return SigmaClient.mc.thePlayer.getDisplayName().getFormattedText().charAt(2) == SigmaClient.fancy && e2.getDisplayName().getFormattedText().charAt(2) == '?' && SigmaClient.mc.thePlayer.getDisplayName().getFormattedText().charAt(3) == e2.getDisplayName().getFormattedText().charAt(3);
    }
