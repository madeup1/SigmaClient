...
import git.sigmaclient.managers.ConfigManager;
...
public class Module {
    ...
    public void riskWarning() {
        if(this.flagType == FlagType.DETECTED) {
            SigmaClient.notificationManager.showNotification("This module is detected, use it carefully", 2500, Notification.NotificationType.WARNING);
        }
    }
    ...
    public static List<Module> getModulesByCategory(final Category c) {
        return (List<Module>) SigmaClient.moduleManager.getModules().stream().filter(module -> module.category == c).collect(Collectors.toList());
    }
