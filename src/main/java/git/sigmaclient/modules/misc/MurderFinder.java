package git.sigmaclient.modules.misc;
...
public class MurderFinder extends Module
{
    ...
    public void assign()
    {
        SigmaClient.murderFinder = this;
    }

    public boolean hasLine(final String line) {
        if (SigmaClient.mc.thePlayer != null && SigmaClient.mc.thePlayer.getWorldScoreboard() != null && SigmaClient.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1) != null) {
    ...
    public void onTick(final TickEvent.ClientTickEvent event) {
        if (!this.isToggled() || SigmaClient.mc.thePlayer == null || SigmaClient.mc.theWorld == null) {
            return;
        }

        try {
            if (SigmaClient.mc.thePlayer.getWorldScoreboard() != null) {
                final ScoreObjective objective = SigmaClient.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1);
                if (objective != null && ChatFormatting.stripFormatting(objective.getDisplayName()).equals("MURDER MYSTERY") && hasLine("Innocents Left:")) {
                    this.inMurder = true;
                    for (final EntityPlayer player : SigmaClient.mc.theWorld.playerEntities) {
                        if (!murderers.contains(player)) {
                            ...
                            if (detectives.size() < 2 && player.getHeldItem().getItem().equals(Items.bow)) {
                                ...
                                SigmaClient.sendMessageWithPrefix(String.format(SigmaClient.fancy + "b%s is detective!", player.getName()));
                                SigmaClient.notificationManager.showNotification(String.format(SigmaClient.fancy + "b%s is detective!", player.getName()), 5000, Notification.NotificationType.WARNING);
                            }
                            if (!this.knives.contains(player.getHeldItem().getItem())) {
                                continue;
                            }
                            murderers.add(player);
                            SigmaClient.sendMessageWithPrefix(String.format(SigmaClient.fancy + "c%s is murderer!", player.getName()));
                            SigmaClient.notificationManager.showNotification(String.format(SigmaClient.fancy + "c%s is murderer!", player.getName()), 10000, Notification.NotificationType.WARNING);
                            if (!this.autoSay.isEnabled() || player == SigmaClient.mc.thePlayer) {
                                continue;
                            }
                            SigmaClient.mc.thePlayer.sendChatMessage(String.format("%s is murderer!", ChatFormatting.stripFormatting(player.getName())));
                        }
                    }
                    return;
                }
    ...
    public void onWorldRender(final RenderWorldLastEvent e) {
        ...
        if (this.inMurder) {
            for (final Entity entity : SigmaClient.mc.theWorld.loadedEntityList) {
                if (entity instanceof EntityPlayer) {
                    if (((EntityPlayer)entity).isPlayerSleeping()) {
                        continue;
                    }
                    if (entity == SigmaClient.mc.thePlayer) {
                        continue;
                    }
