...
public class ChinaHat extends Module
{
    ...
    public void assign()
    {
        SigmaClient.chinaHat = this;
    }

    @SubscribeEvent
    public void onRender(final RenderWorldLastEvent event) {
        if (this.isToggled() && (SigmaClient.mc.gameSettings.thirdPersonView != 0 || this.firstPerson.isEnabled())) {
            this.drawChinaHat((EntityLivingBase) SigmaClient.mc.thePlayer, event.partialTicks);
        }
    }
    ...
    private void drawChinaHat(final EntityLivingBase entity, final float partialTicks) {
        ...
        GL11.glTranslated(entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * partialTicks - SigmaClient.mc.getRenderManager().viewerPosX, entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * partialTicks - SigmaClient.mc.getRenderManager().viewerPosY + entity.height + (entity.isSneaking() ? (this.pos.getValue() - 0.23000000417232513) : this.pos.getValue()), entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * partialTicks - SigmaClient.mc.getRenderManager().viewerPosZ);
        GL11.glRotatef((float)((entity.ticksExisted + partialTicks) * this.rotation.getValue()) - 90.0f, 0.0f, 1.0f, 0.0f);
        GL11.glRotatef(-(SigmaClient.mc.thePlayer.prevRotationYawHead + (SigmaClient.mc.thePlayer.rotationYawHead - SigmaClient.mc.thePlayer.prevRotationYawHead) * partialTicks), 0.0f, 1.0f, 0.0f);
    ...
    public Color getColor(final double i, final double max, final boolean first) {
        ...
        return SigmaClient.clickGui.getColor((int)c);
    }
}
