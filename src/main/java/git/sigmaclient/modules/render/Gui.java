...
public class Gui extends Module {
    ...
    public Gui() {
        ...
        this.colorMode = new ModeSetting("Theme", "Katoz", new String[] { "Katoz", "Gradient", "Rainbow", "Vape", "Mint", "Devil"})
        {
            ...
            public void cycle(int key)
            {
                ...
                SigmaClient.themeManager.setTheme(colorMode.getSelected());
            }
        };
    ...
    public float getHeight() {
        ...
        final List<Module> list = SigmaClient.moduleManager.getModules().stream().filter(module ->  (module.isToggled() || module.toggledTime.getTimePassed() <= 250L)).sorted(Comparator.comparingDouble(module -> Fonts.getPrimary().getStringWidth(module.getName()))).collect(Collectors.toList());
        float y = 3.0f;
    ...
    public void onRender(final RenderGameOverlayEvent.Post event) {
        if (event.type == RenderGameOverlayEvent.ElementType.CROSSHAIRS) {
            if (this.waterMark.isEnabled()) {
                Fonts.getSecondary().drawSmoothString("ore", Fonts.getSecondary().drawSmoothString("K", 5.0, 5.0f, Color.white.darker().getRGB()) + 1.0f, 5.0f, SigmaClient.themeManager.getSecondaryColor(0).getRGB());
            }
            if (this.arrayList.isEnabled()) {
                GL11.glPushMatrix();
                final ScaledResolution resolution = new ScaledResolution(SigmaClient.mc);
                final List<Module> list = SigmaClient.moduleManager.getModules().stream().filter(module -> (module.isToggled() || module.toggledTime.getTimePassed() <= 250L)).sorted(Comparator.comparingDouble(module -> Fonts.getPrimary().getStringWidth(module.getName() + " | " + module.suffix()))).collect(Collectors.toList());
                ...
                for (final Module module2 : list) {
                    ...
                    if (!module2.suffix().isEmpty())
                    {
                        moduleName += " | " + module2.suffix();
                    }
                    ...
                    Fonts.getPrimary().drawSmoothCenteredString(moduleName, resolution.getScaledWidth() - 1 - width / 2.0f, y + height / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, SigmaClient.themeManager.getSecondaryColor(x).getRGB(), SigmaClient.themeManager.getSecondaryColor(x - 1).getRGB());
                ...
                if (this.arrayOutline.isEnabled()) {
                    ...
                    for (final Module module3 : list) {
                        ...
                        addVertex((float)(resolution.getScaledWidth() - 1), y, (float)resolution.getScaledWidth(), y + height, SigmaClient.themeManager.getSecondaryColor(x - 1).getRGB(), SigmaClient.themeManager.getSecondaryColor(x).getRGB());
                        y += height;
                    }
                    tessellator.draw();
    ...
    public void onEnable()
    {
        ...
        SigmaClient.mc.displayGuiScreen(modernClickGui);
    }

    @Override
    public void onDisable()
    {
        SigmaClient.mc.displayGuiScreen(null);
    }

    public Color getColor() {
        return SigmaClient.themeManager.getSecondaryColor(0);
    }

    public Color getColor(int index) {
        return SigmaClient.themeManager.getSecondaryColor(index);
    }
}
