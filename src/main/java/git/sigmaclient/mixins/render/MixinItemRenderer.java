...
public abstract class MixinItemRenderer {
    ...
    public void renderItemInFirstPerson(final float partialTicks) {
        ...
        //final boolean shouldSpoofBlocking = (SigmaClient.killAura.isToggled() && KillAura.target != null && !SigmaClient.killAura.blockMode.getSelected().equals("None"));
        if (this.itemToRender != null) {
            if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                switch (enumaction) {
                    ...
                    case BLOCK: {
                        if (SigmaClient.animations.isToggled()) {
                            final String selected = SigmaClient.animations.mode.getSelected();
                            switch (selected) {
                                case "1.7": {
                                ...
                                case "spin":
                                case "vertical spin": {
                                    this.transformFirstPersonItem(f, SigmaClient.animations.showSwing.isEnabled() ? f2 : 0.0f);
                                    this.doBlockTransformations();
                                    break;
                                }
                                case "long hit": {
    ...
    private void transformFirstPersonItem(final float equipProgress, final float swingProgress) {
        if (SigmaClient.animations.isToggled()) {
            final float size = (float) SigmaClient.animations.size.getValue();
            final float x = (float) SigmaClient.animations.x.getValue();
            final float y = (float) SigmaClient.animations.y.getValue();
            final float z = (float) SigmaClient.animations.z.getValue();
    ...
    private void doBlockTransformations() {
        ...
        final String selected = SigmaClient.animations.mode.getSelected();
        if(SigmaClient.animations != null && SigmaClient.animations.isToggled()) {
            switch (selected) {
                case "vertical spin": {
